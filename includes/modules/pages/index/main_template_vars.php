...
if($_GET['sortby']) {
   switch($_GET['sortby']) {
      default:
         $listing_sql = str_replace('order by', 'order by p.products_sort_order asc, ', $listing_sql);
         break;
      case 'most_popular':
         $listing_sql = str_replace('order by', 'order by p.products_ordered desc, ', $listing_sql);
         break;
      case 'newest_first':
         $listing_sql = str_replace('order by', 'order by p.products_date_added desc, ', $listing_sql);
         break;
      case 'price_l2h':
         $listing_sql = str_replace('order by', 'order by p.products_price_sorter asc, ', $listing_sql);
         break;
      case 'price_h2l':
         $listing_sql = str_replace('order by', 'order by p.products_price_sorter desc, ', $listing_sql);
         break;
      case 'name_az':
         $listing_sql = str_replace('order by', 'order by pd.products_name asc, ', $listing_sql);
         break;
      case 'name_za':
         $listing_sql = str_replace('order by', 'order by pd.products_name desc, ', $listing_sql);
         break;
   }
}
...
// get lastest categories id
$categories_id = $_SESSION['category_tree']->getChildrenCategories($current_category_id);
$flt_products_id = $nmnFilters->getAllProductsFromCategoryId($categories_id);

// replace sql string
$strsearch = "p2c.categories_id = '" . $current_category_id . "'";
$strreplace = "p2c.products_id IN (" . implode(',', $flt_products_id) . ")";
$listing_sql = str_replace('select', 'select distinct p.products_id, ', $listing_sql);
$listing_sql = str_replace($strsearch, $strreplace, $listing_sql);
...
if(isset($_GET['sortby']) && !empty($_GET['sortby'])) {
  ...
  switch($_GET['sortby']) {
    default:
      $strreplace = 'order by p.products_sort_order asc,';
      $listing_sql = str_replace($strsearch, $strreplace, $listing_sql);      
      break;
    case '1':
      $strreplace = 'order by p.products_date_added desc,';
      $listing_sql = str_replace($strsearch, $strreplace, $listing_sql);
      break;
    case '2':
      $strreplace = 'order by p.products_ordered desc,';
      $listing_sql = str_replace($strsearch, $strreplace, $listing_sql);
      break;
    case '3':
      $strreplace = 'order by p.products_price_sorter desc,';
      $listing_sql = str_replace($strsearch, $strreplace, $listing_sql);
      break;
    case '4':
      $strreplace = 'order by p.products_price_sorter asc,';
      $listing_sql = str_replace($strsearch, $strreplace, $listing_sql);
      break;
    case '5':
      $strreplace = 'order by pd.products_name asc,';
      $listing_sql = str_replace($strsearch, $strreplace, $listing_sql);
      break;
    case '6':
      $strreplace = 'order by pd.products_name desc,';
      $listing_sql = str_replace($strsearch, $strreplace, $listing_sql);
      break;
  }
}
