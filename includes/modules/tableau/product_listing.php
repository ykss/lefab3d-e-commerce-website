...
if ($listing_split->number_of_rows > 0 || $_GET['showall'] == 1) {
  $rows = 0;
  ...
  while (!$listing->EOF) {
    ...
    for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
      ...
      switch ($column_list[$col]) {
        ...
        case 'PRODUCT_LIST_NAME':
        $lc_align = '';
				$reviews_average_rating_query = "select avg(r.reviews_rating) as average_rating, count(r.reviews_id) as total_ratings from " . TABLE_REVIEWS . " r 
        ...
        if (isset($_GET['manufacturers_id'])) {
          $lc_text = '
              ...
              <div class="rating-wrap">
                  <span class="note-stars"><img width="87" height="15" alt="" src="' . DIR_WS_TEMPLATE . 'images/stars_' . (int)$reviews_average_rating->fields['average_rating'] . '.gif"></span>
                  <span class="note-numbers">(' . $reviews_average_rating->fields['total_ratings']. ')</span>
              </div>
        ...
        if (zen_has_product_attributes($listing->fields['products_id']) or PRODUCT_LIST_PRICE_BUY_NOW == '0') {
          $lc_button = '<a class="more-info" href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? $_GET['cPath'] : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '">' . MORE_INFO_TEXT . '</a>';
        } else {
          if (PRODUCT_LISTING_MULTIPLE_ADD_TO_CART != 0) {
            ...
            if ($listing->fields['products_qty_box_status'] == 0) {
              ...
              $lc_button = "<div class=\"product-add-cart\"><span>" . TEXT_PRODUCT_LISTING_MULTIPLE_ADD_TO_CART . "</span><input type=\"text\" name=\"products_id[" . $listing->fields['products_id'] . "]\" value=\"0\" size=\"4\" />";
            }
          } else {
// qty box with add to cart button
        ...
        $products_link = '<a class="more-info" href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . ( ($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ? zen_get_generated_category_path_rev($_GET['filter_id']) : $_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id'])) . '&products_id=' . $listing->fields['products_id']) . '">' . MORE_INFO_TEXT . '</a>';
        ...
        if ($listing->fields['products_image'] == '' and PRODUCTS_IMAGE_NO_IMAGE_STATUS == 0) {
          ...
          /*if(zen_get_products_stock($listing->fields['products_id']) == 0){
            $classgrave = 'class="OutOfStock"';
          }*/
          $classgrave = '';
          if(current(explode('_', $_GET['cPath'])) == 26) {
            $classgrave = 'class="graveyard"';
          }
          if (isset($_GET['manufacturers_id'])) {
            $lc_text = '<div class="product-image"><a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '" '.$classgrave.'>' . zen_image(DIR_WS_IMAGES . $listing->fields['products_image'], $listing->fields['products_name'], IMAGE_PRODUCT_LISTING_WIDTH, IMAGE_PRODUCT_LISTING_HEIGHT, 'class="listingProductImage"') . '</a></div>';
          } else {
            $lc_text = '<div class="product-image"><a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? zen_get_generated_category_path_rev($_GET['cPath']) : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '" '.$classgrave.'>' . zen_image(DIR_WS_IMAGES . $listing->fields['products_image'], $listing->fields['products_name'], IMAGE_PRODUCT_LISTING_WIDTH, IMAGE_PRODUCT_LISTING_HEIGHT, 'class="listingProductImage"') . '</a></div>';
          }
        }
    ...
    if (PRODUCT_LISTING_LAYOUT_STYLE == 'columns') {
      ...
      $list_box_contents[$rows][$column] = array('params' => 'class="centerBoxContentsProducts centeredContent ' . $product_listing_style . 'back"' . ' ' . 'style="width:' . $col_width . '%;"',
                                                 'text'  => $lc_text,
                                                 'creation_date' => $listing->fields['products_date_added'],                                                 'product_id' => $listing->fields['products_id']);
      $column ++;
