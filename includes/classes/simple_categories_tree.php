...
class SimpleCategoriesTree{
	...
	function __buildCategoryString($categories_id, $level, $max_level, $strict){
		...
		//Edit from lines 184 to 196
		$defClass = 'no_active';
		
		if ($_GET['cPath'] == 363 || empty($_GET['cPath'])){
			$defClass = 'active1';
		}
		
		$result = '<li class = "level_1 no_sub '. $defClass .'">
			<a class="level_1 no_sub '. $defClass .'" href="/dev/index.php?main_page=index&cPath=363" title="All Departments">
		...
		if(($level < $max_level) || $is_allowed_to_go_deeper){
			...
			if($level > 0){
				...
				if ($this->category_tree[$categories_id]['name'] != 'Brands')
